...
isCouponRelatedQuery(query) {
  ...
  showActionIcons(messageDiv, content) {
    // Remove any existing action icons
    const existingIcons = messageDiv.querySelector('.action-icons');
  ...
  copyMessage(messageDiv) {
    const textToCopy = messageDiv.innerText; // Copy all text inside the message
  ...
  shareMessage(messageDiv) {
    const textToShare = messageDiv.innerText; // Share all text inside the message
    if (navigator.share) {
      navigator.share({
        title: 'Chat Message',
        text: textToShare,
        ...
        .catch((err) => {
          this.showToast('Failed to share message');
        });
    } else {
      // Fallback for devices that don't support the Web Share API
      const shareUrl = `mailto:?subject=Chat Message&body=${encodeURIComponent(textToShare)}`;
